...
/**
 * Reading of messages using data in databases. It must be provided with a datasource and sql code
 * which must be passed 2 jdbc parameters which will be respectively filled with the code and the
 * string representation of the locale. <BR>
 */
@Slf4j
...
//		return resolveCodeInternal(code, locale);
   ...
   *
   * @param code the code of the message to solve
   * @param locale the locale to check against
   * @return a MessageFormat if one were found, either for the given locale or for the default on,
  ...
  /**
   * Empty the cache so that all future message resolving request ends in database queries
   */
